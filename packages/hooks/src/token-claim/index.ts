...
export async function claimToken(eventId: string, storageId: string, identity: ActorIdentity) {
  return resultFormat<boolean>(await (await tokenClaimStorage(storageId, identity)).userClaim(eventId));
}
