...
import type { PaginationResult, WrapUser, WrapTransaction } from "@w2e/types";
...
export async function getWrapTransactions(
  ...
  return await (
    ...
    user: account
      ? availableArgsNull<WrapUser>(isPrincipal(account) ? { principal: account } : { address: account })
      : [],
...
export function useWrapTransactions(
  ...
  return useCallsData(
    useCallback(async () => {
      ...
      return resultFormat<PaginationResult<WrapTransaction>>(await getWrapTransactions(account, offset, limit, index))
        .data;
