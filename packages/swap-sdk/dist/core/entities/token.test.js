    ...
    describe("#constructor", () => {
        it("fails with invalid address", () => {
            expect(() => new Token({
                ...
                decimals: 18,
                standard: "EXT",
            }).address).toThrow("0x_hello_00000000000000000000000000000000002 is not a valid address");
        });
        it("fails with negative decimals", () => {
            expect(() => new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: -1, standard: "EXT" }).address).toThrow("DECIMALS");
        });
        it("fails with 256 decimals", () => {
            expect(() => new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 256, standard: "EXT" }).address).toThrow("DECIMALS");
        });
        it("fails with non-integer decimals", () => {
            expect(() => new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 1.5, standard: "EXT" }).address).toThrow("DECIMALS");
        });
    });
    describe("#equals", () => {
        it("fails if address differs", () => {
            expect(new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 18, standard: "EXT" }).equals(new Token({ symbol: "A", name: "A", address: ADDRESS_TWO, decimals: 18, standard: "EXT" }))).toBe(false);
        });
        it("true if only decimals differs", () => {
            expect(new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 9, standard: "EXT" }).equals(new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 18, standard: "EXT" }))).toBe(true);
        });
        it("true if address is the same", () => {
            expect(new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 18, standard: "EXT" }).equals(new Token({ symbol: "A", name: "A", address: ADDRESS_ONE, decimals: 18, standard: "EXT" }))).toBe(true);
        });
