...
import { Fraction, Percent, Price, sortedInsert, CurrencyAmount, TradeType } from "../core";
...
export class Trade {
    ...
    get executionPrice() {
        ...
        return new Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient);
    }
    /**
     ...
     * @returns The exact in trade
     */
    static bestTradeExactIn(pools_1, currencyAmountIn_1, currencyOut_1) {
        return __awaiter(this, arguments, void 0, function* (pools, currencyAmountIn, currencyOut, { maxNumResults = 3, maxHops = 3 } = {}, 
        // used in recursion.
        currentPools = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
            ...
            for (let i = 0; i < pools.length; i++) {
                ...
                if (amountOut.currency.isToken && amountOut.currency.equals(tokenOut)) {
                    sortedInsert(bestTrades, yield Trade.fromRoute(new Route([...currentPools, pool], currencyAmountIn.currency, currencyOut), currencyAmountIn, TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
                }
     ...
     * @returns The exact out trade
     */
    static bestTradeExactOut(pools_1, currencyIn_1, currencyAmountOut_1) {
        return __awaiter(this, arguments, void 0, function* (pools, currencyIn, currencyAmountOut, { maxNumResults = 3, maxHops = 3 } = {}, 
        // used in recursion.
        currentPools = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
            ...
            for (let i = 0; i < pools.length; i++) {
                ...
                if (amountIn.currency.equals(tokenIn)) {
                    sortedInsert(bestTrades, yield Trade.fromRoute(new Route([pool, ...currentPools], currencyIn, currencyAmountOut.currency), currencyAmountOut, TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
                }
                else if (maxHops > 1 && pools.length > 1) {
                    const poolsExcludingThisPool = pools.slice(0, i).concat(pools.slice(i + 1, pools.length));
